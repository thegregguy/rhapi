#!/usr/bin/env python3
"""
Auto Trader CLI - Interactive cryptocurrency trading bot

This is the main entrypoint that provides a menu-based interface
for the modular trader system.

Backward-compatible with the original auto_trader.py interface.
"""
...
def run_diagnostics():
    """Run system diagnostics to check API connectivity and configuration."""
    clear_screen()
...
def run_trading_loop():
    """Start the automated trading loop using the Trader class."""
    clear_screen()
    
    # Create trader with config from environment
    config = Config()
    trader = Trader(config)
    
    # Run the loop (handles KeyboardInterrupt internally)
    trader.run_loop()
